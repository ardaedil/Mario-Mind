...
## Baseline Agents Before Training

Before training DQN/DDQN, run non-learning baselines to validate environment interaction, action mappings, and reward/termination signals.

Why baselines matter:
- They establish a reference point for reward and progress.
- They catch wrapper/action-space bugs early.
- They confirm metrics collection is working before expensive training.

`AlwaysRightAgent` is a key sanity check because many side-scroller setups should show meaningful rightward progress even with a trivial policy.

### Run baselines on dummy environment
```bash
python scripts/run_baselines.py --env dummy --episodes 20 --output-dir results/baselines/dummy
```

### Run baselines on Mario environment
```bash
...
### Outputs to inspect before DQN/DDQN
- `metrics.csv` (episode-level metrics)
- `summary.csv` (per-agent aggregate stats)
- `steps.csv` (optional per-step trajectory logs)
- `baseline_report.md` (human-readable report)

Key metrics:
- reward trends
- max/final x-position
- success/death/timeout rates
- episode length
- action usage patterns

### Mario setup helper (recommended first)
```bash
python scripts/check_mario_setup.py
```

For full instructions see `docs/real_mario_setup.md`.
